...
// GET /api/n8n/social/facebook?limit=5 — published articles not yet posted to Facebook
export async function GET(req: NextRequest) {
  if (!authCheck(req)) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })

  const limit = Math.min(Number(req.nextUrl.searchParams.get('limit') ?? 5), 20)

  // Step 1: collect article IDs already successfully posted to Facebook
  ...
  // Step 2: published articles not in that set
...
// PATCH /api/n8n/social/facebook — record result after Facebook post attempt
